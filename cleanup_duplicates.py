#!/usr/bin/env python3
import os
import sys
import re
import json
import time
import ssl
import urllib.request
import urllib.parse
from collections import defaultdict

ENV_FILE = ".env.discogs"
CATALOG_FILE = "catalog_2026-07-26.md"
ssl_ctx = ssl._create_unverified_context()

def load_env(env_path=ENV_FILE):
    env = {}
    if os.path.exists(env_path):
        with open(env_path, "r", encoding="utf-8") as f:
            for line in f:
                line = line.strip()
                if line and not line.startswith("#") and "=" in line:
                    k, v = line.split("=", 1)
                    env[k.strip()] = v.strip()
    return env

def parse_username(env):
    if "Username" in env:
        return env["Username"]
    if "User_Profile" in env:
        parts = env["User_Profile"].rstrip("/").split("/")
        return parts[-1]
    return None

def parse_catalog(filepath=CATALOG_FILE):
    items = []
    if not os.path.exists(filepath):
        return items
    with open(filepath, "r", encoding="utf-8") as f:
        for line in f:
            line = line.strip()
            if not line.startswith("* **"):
                continue
            m = re.match(r"^\*\s*\*\*(.*?)\*\*\s*[\u2013\u2014\-]\s*(.*)$", line)
            if not m:
                continue
            artist = m.group(1).strip()
            rest = m.group(2).strip()
            
            count = 1
            count_match = re.search(r"\((\d+)\s+copies(?:\/pressings)?\)", rest, re.IGNORECASE)
            if count_match:
                count = int(count_match.group(1))
            
            albums = re.findall(r"\*(.*?)\*", rest)
            if not albums:
                clean_rest = re.sub(r"\s*\([^)]*\)\s*$", "", rest)
                albums = [clean_rest]
            
            for album in albums:
                album_clean = album.strip()
                if album_clean:
                    items.append({
                        "artist": artist,
                        "album": album_clean,
                        "count": count
                    })
    return items

def make_request(url, auth_header, method="GET"):
    headers = {
        "User-Agent": "DiscogsDuplicateCleanupScript/1.0",
        "Authorization": auth_header
    }
    req = urllib.request.Request(url, headers=headers, method=method)
    for attempt in range(5):
        try:
            with urllib.request.urlopen(req, context=ssl_ctx) as resp:
                remaining = resp.headers.get("X-Discogs-Ratelimit-Remaining")
                if remaining is not None:
                    try:
                        if int(remaining) < 3:
                            time.sleep(10)
                    except ValueError:
                        pass
                if method == "DELETE":
                    return True
                return json.loads(resp.read().decode("utf-8"))
        except urllib.error.HTTPError as e:
            if e.code in (200, 204) and method == "DELETE":
                return True
            if e.code == 429:
                retry_after = e.headers.get("Retry-After")
                wait_time = int(retry_after) if retry_after and retry_after.isdigit() else 60
                print(f"\n[Rate Limit] Pausing {wait_time}s...", flush=True)
                time.sleep(wait_time)
                continue
            raise Exception(f"HTTP {e.code} {e.reason}")
    raise Exception("Max retries exceeded")

def clean_artist_name(name):
    return re.sub(r"\s*\(\d+\)$", "", name).strip()

def normalize_key(artist, album):
    a_clean = clean_artist_name(artist).lower()
    t_clean = album.lower()
    a_clean = re.sub(r"[^\w\s]", "", a_clean).strip()
    t_clean = re.sub(r"[^\w\s]", "", t_clean).strip()
    return (a_clean, t_clean)

def fetch_all_collection_items(username, auth_header):
    print(f"Fetching collection items for user '{username}' from Discogs...")
    items = []
    page = 1
    total_pages = 1
    
    while page <= total_pages:
        url = f"https://api.discogs.com/users/{username}/collection/folders/0/releases?per_page=100&page={page}"
        print(f"  Fetching page {page}/{total_pages}...", end=" ", flush=True)
        data = make_request(url, auth_header)
        
        pagination = data.get("pagination", {})
        total_pages = pagination.get("pages", 1)
        
        releases = data.get("releases", [])
        items.extend(releases)
        print(f"Got {len(releases)} items (Total fetched: {len(items)})")
        
        page += 1
        time.sleep(1.0)
        
    return items

def analyze_duplicates(items):
    grouped = defaultdict(list)
    
    for item in items:
        basic = item.get("basic_information", {})
        artists = basic.get("artists", [])
        artist_name = ", ".join([a.get("name", "") for a in artists]) if artists else "Unknown Artist"
        title = basic.get("title", "Unknown Title")
        release_id = basic.get("id")
        instance_id = item.get("instance_id")
        folder_id = item.get("folder_id", 1)
        formats = basic.get("formats", [])
        fmt_str = ", ".join([f.get("name", "") for f in formats]) if formats else ""

        key = normalize_key(artist_name, title)
        
        grouped[key].append({
            "artist": clean_artist_name(artist_name),
            "title": title,
            "release_id": release_id,
            "instance_id": instance_id,
            "folder_id": folder_id,
            "format": fmt_str,
            "date_added": item.get("date_added", "")
        })
        
    duplicates = {k: v for k, v in grouped.items() if len(v) > 1}
    return grouped, duplicates

def get_allowed_count(artist, title, catalog_items):
    norm_key = normalize_key(artist, title)
    for cat in catalog_items:
        c_key = normalize_key(cat["artist"], cat["album"])
        if norm_key == c_key:
            return cat["count"]
    return 1

def delete_instance(username, folder_id, release_id, instance_id, auth_header):
    url = f"https://api.discogs.com/users/{username}/collection/folders/{folder_id}/releases/{release_id}/instances/{instance_id}"
    return make_request(url, auth_header, method="DELETE")

def main():
    execute_mode = "--execute" in sys.argv or "--force" in sys.argv
    
    env = load_env()
    token = env.get("Personal_Token") or env.get("DISCOGS_TOKEN")
    username = parse_username(env)
    
    if not token or not username:
        print("Error: Personal_Token or Username missing in .env.discogs")
        sys.exit(1)
        
    auth_header = f"Discogs token={token}"
    catalog_items = parse_catalog()
    
    items = fetch_all_collection_items(username, auth_header)
    grouped, duplicates = analyze_duplicates(items)
    
    if not duplicates:
        print("\n🎉 No duplicate album pairs found in your Discogs collection!")
        return

    to_delete = []
    to_keep = []

    for key, dup_list in duplicates.items():
        artist = dup_list[0]["artist"]
        title = dup_list[0]["title"]
        allowed_count = get_allowed_count(artist, title, catalog_items)
        
        # Sort items by date_added ascending (oldest first)
        # So we keep the oldest entries and remove redundant newer ones
        sorted_dups = sorted(dup_list, key=lambda x: x["date_added"])
        
        keep_items = sorted_dups[:allowed_count]
        del_items = sorted_dups[allowed_count:]
        
        to_keep.extend(keep_items)
        to_delete.extend(del_items)

    print(f"\n==========================================")
    print(f" DISCOGS DUPLICATE CLEANUP REPORT")
    print(f" Mode: {'EXECUTE (Deleting duplicates)' if execute_mode else 'DRY RUN (Previewing deletions)'}")
    print(f" Duplicate Groups Identified: {len(duplicates)}")
    print(f" Instances to Delete: {len(to_delete)}")
    print(f"==========================================\n")

    print("Plan for Deletions:\n")
    for item in to_delete:
        print(f" ❌ DELETE: {item['artist']} - '{item['title']}'")
        print(f"    • Instance ID: {item['instance_id']} | Release ID: {item['release_id']} | Added: {item['date_added']}")
    
    if not execute_mode:
        print("\n-------------------------------------------------------------")
        print("ℹ️ This was a DRY RUN. No items were deleted.")
        print("To perform the actual deletion, run:")
        print("   python3 cleanup_duplicates.py --execute")
        print("-------------------------------------------------------------")
        return

    print("\nStarting automated deletion of duplicates from Discogs...")
    deleted_count = 0
    for idx, item in enumerate(to_delete, 1):
        print(f"[{idx}/{len(to_delete)}] Deleting Instance {item['instance_id']} ({item['artist']} - '{item['title']}')...", end=" ", flush=True)
        try:
            delete_instance(username, item["folder_id"], item["release_id"], item["instance_id"], auth_header)
            print("✅ DELETED")
            deleted_count += 1
            time.sleep(1.2)
        except Exception as e:
            print(f"❌ ERROR: {e}")

    print(f"\n🎉 Cleanup Complete! Successfully deleted {deleted_count} duplicate instances.")

if __name__ == "__main__":
    main()
